...
#default run.txt to 0 so it runs after boot
file = open("/home/pi/tf_inference/run.txt", "w")
file.write("0")
file.close()
...
def send_everyfour():
          threading.Timer(14400.0,send_everyfour).start()
	  file = open("/home/pi/tf_inference/run.txt", "w")
	  file.write("0")
	  file.close()
...
while True:
	...
	elif(data=="ADRNSET\n"): # will block the script from running
         file = open("/home/pi/tf_inference/run.txt", "w")
         file.write("1")
         file.close()
         ser.write("D "+host+" " + str(ble_address)+ "RUNST" + "\n")
        elif(data=="ADRNRST\n"): # will block the script from running
         file = open("/home/pi/tf_inference/run.txt", "w")
         file.write("0")
         file.close()
         ser.write("D "+host+" " + str(ble_address)+ "RUNRT" + "\n")
